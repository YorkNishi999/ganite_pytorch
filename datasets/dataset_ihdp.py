...
from .network import download_http_if_needed
...
def load_raw(data_path = "../data") -> Tuple:
    """
    ...
    train_csv = os.join.path(data_path, TRAIN_DATASET)
    test_csv = os.join.path(data_path, TEST_DATASET)

    print(f"load raw dataset f{train_csv}")

    download_http_if_needed(train_csv, http_url=TRAIN_URL)
    download_http_if_needed(test_csv, http_url=TEST_URL)
